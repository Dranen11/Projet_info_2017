...
//Solving a symmetric linear system using cholesky decomposition
...
//Solving a symmetric linear system using cholesky decomposition in the case of the subdiagonal have always the same value
template<typename Type>
Type* solve_cholesky_tri_diag(Type low_diag, Type* diag, Type* u, std::size_t n)
{
   Type *result, *dec_diag, *dec_low_diag; //Variables dec are think to store the decomposition
   ...
   for(size_t i = 0; i < (n-1); i++)
   {
      dec_low_diag[i] = low_diag/dec_diag[i];
      dec_diag[i+1] = sqrt(diag[i+1]-dec_low_diag[i]*dec_low_diag[i]);
   }
