...
{
   complex<double> alpha = 1.0i*H_BAR/(2.0*m), beta = alpha/(2.0*dx*dx), prov;
   ...
   prov = potential(0, t, param_potential)/(2.0*1.0i*H_BAR);
   lambda[0] = (1.0/dt)+2.0*beta-prov;
   u[0] = position[0] * ((1.0/dt)-2.0*beta+prov) + beta * position[1];
   sub_diag[0] = beta;
   for(size_t i = 1; i < (N_x-1); i++)
   {
      prov = potential(i*dx, t, param_potential)/(2.0*1.0i*H_BAR);
      lambda[i] = (1.0/dt)+2.0*beta-prov;
      u[i] = beta*position[i-1] + position[i]*((1.0/dt)-2.0*beta+prov) + beta*position[i+1];
      sub_diag[i] = beta;
   }
   prov = potential((N_x-1)*dx, t, param_potential)/(2.0*1.0i*H_BAR);
   lambda[N_x-1] = (1.0/dt)+2.0*beta-prov;
   u[N_x-1] = position[N_x-1] * ((1.0/dt)-2.0*beta+prov) + beta * position[N_x-2];
...
void Crank_Nicholson_1D(size_t N_x, double dx, double dt, double m, complex<double>* position, double *potential)
{
   ...
   prov = potential[0]/(2.0*1.0i*H_BAR);
   lambda[0] = (1.0/dt)+2.0*beta-prov;
   u[0] = position[0] * ((1.0/dt)-2.0*beta+prov) + beta * position[1];
   sub_diag[0] = beta;
   for(size_t i = 1; i < (N_x-1); i++)
   {
      prov = potential[i]/(2.0*1.0i*H_BAR);
      lambda[i] = (1.0/dt)+2.0*beta-prov;
      u[i] = beta*position[i-1] + position[i]*((1.0/dt)-2.0*beta+prov) + beta*position[i+1];
      sub_diag[i] = beta;
   }
   prov = potential[N_x-1]/(2.0*1.0i*H_BAR);
   lambda[N_x-1] = (1.0/dt)+2.0*beta-prov;
   u[N_x-1] = position[N_x-1] * ((1.0/dt)-2.0*beta+prov) + beta * position[N_x-2];
